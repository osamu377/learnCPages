...
![一目でわかる！C言語の配列](p1c3.png)

## イントロダクション

この章でを終える頃には、あなたは複数のデータを効率的に扱うための「配列（Array）」を完全にマスターし、C言語で最も強力な機能の一つである「ポインタ」の学習に進むための揺るぎない土台を築くことができるようになります。一つずつ、着実に自分のスキルにしていきましょう。
...
配列の基本から応用までを学んできましたが、これらの知識はすべて、C言語の核心へと繋がっています。最後に、本研修のまとめと、次のステップについてお話しします。
...
この研修資料では、C言語における配列の概念を多角的に学んできました。ここで、核心となるコンセプトを振り返りましょう。
...
ポインタとは、データの「値」そのものではなく、そのデータがメモリ上のどこに格納されているかという「場所（住所）」を直接扱うための仕組みです。そして、配列の名前自体が、実はポインタと密接な関係にあることを知ったとき、C言語の全体像がより鮮明に見えてくるはずです。配列の知識を武器に、次のステップであるポインタの世界へ挑戦してください。
