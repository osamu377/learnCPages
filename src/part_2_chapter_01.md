...
`int * pointer_to_a`という宣言は、「デリファレンス（間接参照）したときには、int型の値を扱うことになるポインタ変数」という意味だと思えば、二つの文脈で同じ`*`が使われている理由がわかるでしょう。

だからと言って、`pointer_to_a`という変数は、常に`*`とセットで使われる、というわけではありません。その変数に入っているのは、あくまでメモリ・アドレスであることを忘れないでください。
