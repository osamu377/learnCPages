...
「再帰」のイメージをつかむための、参考として、同じロジックを図形に繰り返し適用する例を見てみましょう。直接、本章に関わるものではありませんが、「再帰」の例には違いありません。

下の図は、「コッホ曲線」と呼ばれている図形を作る過程を示しています。

![(コッホ曲線)](img/How_to_make_Koch_curve.svg)

*Kkairri (original made by Solkoll~commonswiki) パブリック・ドメイン https://commons.wikimedia.org/w/index.php?curid=42886595による*

これは1本の直線に対し、

* 線を3等分して、中央の線を上に引っ張り上げるようにして正三角形にする
* その結果、出来上がった4本の線に対して、同じ変形ロジックを適用する

という操作を3回繰り返して行った結果を示しています（これを無限回繰り返すと、コッホ曲線が出来ます）。
