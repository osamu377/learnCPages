...
これに対し、本稿で詳解する動的メモリ確保は、ヒープと呼ばれる広大で柔軟なメモリプールから、プログラムの実行時に必要なメモリを確保する手法です。この能力こそがC言語のパワーの源泉であり、**実行時までサイズが不確定なデータを扱ったり、複雑なデータ構造を構築したりするため**の、不可欠な戦略的基盤となります。

スタックとヒープなどのメモリ管理については、後の章で、詳しく説明します。ここでは 

1. **必要なサイズのメモリを実行時に確保する**
1.  **不要になったら解放する**

というテクニックをしっかり学んでください。

### 1.1 動的メモリ確保とは何か

動的メモリ確保とは、プログラムの実行中に、任意のサイズのメモリブロックを確保・解放する技術です。このアプローチの核心的な利点は、プログラムを作成した時点ではデータのサイズを事前に知らなくても、データを保存できる点にあります。ユーザー入力、ファイルの内容、ネットワーク通信など、コンパイル時には予測不可能な情報量に対応する上で、この技術は絶対不可欠です。

### 1.2 なぜ動的確保が必要か

動的メモリ確保の真価は、リンクリストのような複雑なデータ構造を構築する際に最も明確に現れます。固定長の配列を確保する静的なアプローチでは、要素数を事前に決定しなければなりません。しかし動的確保を用いれば、必要に応じて新たな要素（ノード）を一つずつ生成し、それらをポインタで連結していくことができます。この技術がもたらす柔軟性と拡張性により、静的確保では実現不可能な、真に可変長のデータ構造をエレガントに実装できるのです。
