...
動的メモリ確保とは、プログラムの実行中に、ヒープ領域から任意のサイズのメモリブロックを確保・解放する技術です。このアプローチの核心的な利点は、プログラムを作成した時点ではデータのサイズを事前に知らなくても、データを保存できる点にあります。ユーザー入力、ファイルの内容、ネットワーク通信など、コンパイル時には予測不可能な情報量に対応する上で、この技術は絶対不可欠です。

### 1.2 なぜ動的確保が必要か

動的メモリ確保の真価は、リンクリストのような複雑なデータ構造を構築する際に最も明確に現れます。スタック上に固定長の配列を確保する静的なアプローチでは、要素数を事前に決定しなければなりません。しかし動的確保を用いれば、ヒープ上に必要に応じて新たな要素（ノード）を一つずつ生成し、それらをポインタで連結していくことができます。この技術がもたらす柔軟性と拡張性により、静的確保では実現不可能な、真に可変長のデータ構造をエレガントに実装できるのです。

動的確保の基本概念を理解したところで、次にその実現に不可欠なツール群について見ていきましょう。

## 2. 動的確保を支える3つのコア要素

動的メモリ確保のライフサイクルは、「確保」「利用」「解放」の3フェーズで構成されます。C言語は、このライフサイクルをプログラマの管理下に置くための3つの主要な構成要素、すなわち malloc、sizeof、free を提供します。これらのツールは互いに連携し、ヒープメモリを安全かつ効率的に管理する仕組みを支えています。

### 2.1 メモリの確保: malloc 関数

malloc（memory allocation）は、ヒープ領域からメモリを確保する、動的確保の心臓部となる関数です。

* 目的: 指定されたバイトサイズのメモリ空間をヒープから予約し、そのブロックへのアクセス手段を提供します。
...
極めて重要なのは、free がポインタ変数自体（例: myperson）を削除するわけではないという事実です。解放後も myperson 変数は存在し続け、解放済みで無効となったアドレスを指したままになります。
...
2. malloc(...): 1で計算されたサイズのメモリブロックをヒープから確保し、その先頭アドレスを void * 型で返します。
