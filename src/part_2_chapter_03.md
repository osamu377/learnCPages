...
2. 参照渡しの実現
    * 関数が呼び出し元の変数を直接変更する必要がある場合、「参照渡し」を用います。これは、アドレス演算子 (`&`) を使って変数のメモリアドレスを渡し、関数側ではポインタ変数 (`*P`) を使ってそのアドレスを受け取り、間接参照演算子 (`*`) を使ってデリファレンスすることで実現します。
3. 構造体への応用
