# 記憶クラス指定子 auto と static の徹底解説

# イントロダクション

ここでは、C言語における変数の **生存期間** と **有効範囲** という、プログラムの安定性を左右する重要な概念について学びます。特に、新人プログラマがつまずきやすい`auto`と`static`という2つの記憶クラス指定子に焦点を当てます。これらの違いを正確に理解し、適切に使い分けることは、意図しないバグを防ぎ、信頼性と保守性の高いコードを書くための不可欠なスキルです。この資料を通じて、変数の振る舞いを深く理解し、より高度なプログラミングへの第一歩を踏み出しましょう。
...
static変数には、主に「関数をまたいで値を保持する」使い方と、「グローバル変数のスコープをファイル内に限定する」という2つの重要な役割があります。次のセクションでは、これらの特性が実際のコードでどのように活かされるのかを具体的に見ていきます。
...
理論と具体例を通じて、両者の違いが明確になったことでしょう。最後に、知識を定着させるための演習問題に取り組んでみましょう。
...
前章で、メモリ空間にはスタック領域とヒープ領域があることを学びました。では、この章で学んだ静的(static)変数や、関数の外で宣言されるグローバル変数は、どちらの領域に入るのでしょうか？

どちらでもない、というのが正解です。

実は、プログラムが利用する主なメモリ領域は以下の4つになります。
...
この全体像を把握することが、メモリ管理を理解する上での第一歩となります。
