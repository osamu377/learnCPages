...
* **`Drink`** は「3つの部屋を持つ棚」です。
* 各部屋には「文字列の先頭住所（ポインタ）」が入っています。
* `Drink[0]` → "Tea" の 'T' の場所
* `Drink[1]` → "Coffee" の 'C' の場所
* `Drink[2]` → "Cocoa" の 'C' の場所
...
* **動作:** `"Cocoa"` の **'o'** を表示します。
* **考え方（1次元的な操作）:**
1. まず、特定の文字列（この場合は `Drink[2]` すなわち `"Cocoa"`）を普通の1次元のポインタ `p` に取り出します。

|インデックス|配列の中身|0|1|2|3|4|5|6||
|:--|:--|:--|:--|:--|:--|:--|:--|:--|:--|
|**2**|"Cocoa" の 'C' のアドレス|C|o|c|o|a|\0||**この行だけを取り出す！**|

2. 「特定の文字列（単語）の中」だけで、アドレスを `+1` して横に移動します。
...
* **動作:** `"Coffee"` の **'f'** を表示します。
* **考え方（2次元配列的な操作）:**
1. `Drink[1]` で「2番目の単語（Coffee）」を指定します。
2. `[3]` でその単語内の「4文字目（'f'）」を直接指定します。
...
2. `(pp + 2)` で「棚の3つ目の部屋（Drink[2]）」まで**縦に移動**します。
...
3. 最初の `*` で「部屋の中にある住所（"Cocoa"の先頭アドレス）」を取り出します。
